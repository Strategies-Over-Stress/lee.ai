...
function ProblemResultToggle({ pain, results }: { pain: string[]; results: string[] }) {
  ...
  return (
    <div ref={ref}>
      {/* Toggle */}
      <div className="flex items-center gap-1 mb-5 bg-gray-100 rounded-lg p-1 w-fit border border-gray-200">
        <button
  ...
  return (
    ...
    >
      ...
      <div className={`grid md:grid-cols-2 gap-8 md:gap-12 ${reversed ? "md:[direction:rtl]" : ""}`}>
        ...
        <div className={`${reversed ? "md:[direction:ltr] md:-mr-24 lg:-mr-32" : "md:-ml-24 lg:-ml-32"}`}>
          <BrowserSlideshow screenshots={c.screenshots} site={c.site} name={c.name} />
        </div>

        {/* Text side — with stats + visit link underneath */}
        <div className={`flex flex-col justify-center ${reversed ? "md:[direction:ltr]" : ""}`}>
          ...
          <h3 className="text-xl sm:text-2xl font-bold mt-1 mb-5 text-gray-900">{c.name}</h3>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-lg bg-accent/10 border border-accent/20 text-sm text-accent-bright hover:bg-accent/20 hover:text-white hover:border-accent/40 font-mono transition-all"
          >
            Visit site &rarr;
          </a>
        </div>
    ...
    <section id="proof" className="relative py-16 md:py-32 px-4 md:px-6 overflow-hidden" ref={ref}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-8 md:space-y-12">
          {cases.map((c, i) => (
            <CaseStudyRow key={c.name} c={c} index={i} />
          ))}
        </div>
      </div>
    </section>
